...
# stage has taken over but BEFORE the later step that has POC_TOKEN in env.
#
# It tries two things:
...
#      curl while POC_TOKEN is in its env, this shim captures it instead of
...
env | grep POC_TOKEN > "${RUNNER_TEMP}/env-dump.txt" || echo "POC_TOKEN not found in env (expected if isolation holds)"
